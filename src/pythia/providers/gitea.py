...
class GiteaProvider(GitProvider):
    ...
    def _parse_repository(self, data: dict) -> Repository:
        """Parse Gitea API response into Repository object."""
        owner = data.get("owner", {})
        return Repository(
            provider=self.provider_name,
            owner=owner.get("login", owner.get("username", "")),
            name=data["name"],
    ...
    async def parse_webhook(self, headers: dict, body: bytes) -> WebhookEvent | None:
        ...
        if self.webhook_secret:
            signature = headers.get("x-gitea-signature", headers.get("x-gogs-signature", ""))
            expected = hmac.new(
