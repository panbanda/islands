...
use pythia_providers::{GitProvider, Repository, WebhookEvent};

#[cfg(feature = "embeddings")]
use pythia_core::embedding::{CloudProvider, EmbedderConfig, EmbedderProvider, InferenceBackend};
...
impl Default for IndexerConfig {
    fn default() -> Self {
        Self {
            repos_path: PathBuf::from("/data/pythia/repos"),
            indexes_path: PathBuf::from("/data/pythia/indexes"),
...
pub struct StoredIndex {
    ...
    pub graph: pythia_core::HnswGraph,
...
impl IndexerService {
    ...
    async fn build_index_with_embeddings(
        ...
        let mut graph = pythia_core::HnswGraph::new(pythia_core::HnswConfig::default())
            .map_err(|e| Error::IndexingFailed(e.to_string()))?;
    ...
    fn build_index_placeholder(
        ...
        let mut graph = pythia_core::HnswGraph::new(pythia_core::HnswConfig::fast())
            .map_err(|e| Error::IndexingFailed(e.to_string()))?;

        for (_path, _content) in files {
            let embedding = pythia_core::embedding::Embedding::zeros(384);
            graph
...
mod tests {
    ...
    fn test_indexer_config_default() {
        ...
        assert_eq!(config.repos_path, PathBuf::from("/data/pythia/repos"));
        assert_eq!(config.indexes_path, PathBuf::from("/data/pythia/indexes"));
    ...
    fn test_index_info_serialization() {
        let repo = pythia_providers::Repository::new(
            "github",
            "test",
            "repo",
