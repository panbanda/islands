...
use crate::tools::PythiaTools;

/// MCP Server for Pythia
...
impl McpServer {
    ...
    pub fn new(indexer: Arc<IndexerService>) -> Self {
        let tools = PythiaTools::new(Arc::clone(&indexer));
    ...
    pub async fn run_stdio(&mut self) -> Result<()> {
        ...
        info!("Pythia MCP server starting on stdio");
...
mod tests {
    ...
    async fn test_handle_initialize() {
        ...
        assert_eq!(server_info.get("name").unwrap(), "pythia");
    }
    ...
    async fn test_handle_tools_list() {
        ...
        assert!(tool_names.contains(&"pythia_search"));
        assert!(tool_names.contains(&"pythia_add_repo"));
        assert!(tool_names.contains(&"pythia_list"));
    }
    ...
    async fn test_initialize_result_structure() {
        ...
        let server_info = result.get("serverInfo").unwrap();
        assert_eq!(server_info.get("name").unwrap().as_str().unwrap(), "pythia");
