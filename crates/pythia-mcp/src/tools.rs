//! MCP Tool implementations for Pythia
...
impl PythiaTools {
    ...
    pub fn list_tools(&self) -> Vec<Tool> {
        vec![
            ...
            Tool {
                name: "pythia_add_repo".to_string(),
    ...
    pub async fn call_tool(&self, name: &str, arguments: Value) -> Result<CallToolResult> {
        match name {
            "pythia_list" => self.handle_list().await,
            "pythia_search" => self.handle_search(arguments).await,
            "pythia_add_repo" => self.handle_add_repo(arguments).await,
            "pythia_sync" => self.handle_sync(arguments).await,
            "pythia_status" => self.handle_status(arguments).await,
            _ => Err(Error::ToolNotFound(name.to_string())),
        }
    }

    /// Handle pythia_list tool
        ...
        if indexes.is_empty() {
            return Ok(CallToolResult {
                content: vec![ContentItem::text(
                    "No indexes available. Add repositories using pythia_add_repo.",
                )],
                is_error: false,
            });
        }

        let mut output = String::from("Available Pythia Indexes:\n\n");
    ...
    async fn handle_add_repo(&self, arguments: Value) -> Result<CallToolResult> {
        ...
        let (provider_type, owner, name, _base_url) =
            pythia_providers::factory::parse_repo_url(&args.url)?;
    ...
    async fn handle_sync(&self, arguments: Value) -> Result<CallToolResult> {
        ...
        let info = self
            ...
            .await
            .ok_or_else(|| {
                Error::Indexer(pythia_indexer::Error::index_not_found(&args.index_name))
            })?;
...
mod tests {
    ...
    #[test]
    fn test_pythia_tools_new() {
        let indexer = create_test_service();
        let _tools = PythiaTools::new(indexer);
    }

    #[test]
    fn test_list_tools() {
        let indexer = create_test_service();
        let tools = PythiaTools::new(indexer);
        let list = tools.list_tools();

        assert_eq!(list.len(), 5);

        let names: Vec<_> = list.iter().map(|t| t.name.as_str()).collect();
        assert!(names.contains(&"pythia_list"));
        assert!(names.contains(&"pythia_search"));
        assert!(names.contains(&"pythia_add_repo"));
        assert!(names.contains(&"pythia_sync"));
        assert!(names.contains(&"pythia_status"));
    }
    ...
    #[test]
    fn test_pythia_search_schema() {
        let indexer = create_test_service();
        let tools = PythiaTools::new(indexer);
        let list = tools.list_tools();

        let search_tool = list.iter().find(|t| t.name == "pythia_search").unwrap();
    ...
    async fn test_handle_list_empty() {
        ...
        let result = tools.call_tool("pythia_list", json!({})).await.unwrap();
    ...
    async fn test_handle_search_empty() {
        ...
        let result = tools
            .call_tool("pythia_search", json!({"query": "test"}))
            .await
            .unwrap();
    ...
    async fn test_handle_search_invalid_params() {
        ...
        let result = tools.call_tool("pythia_search", json!({})).await;
    ...
    async fn test_handle_status_all() {
        ...
        let result = tools.call_tool("pythia_status", json!({})).await.unwrap();
    ...
    async fn test_handle_status_not_found() {
        ...
        let result = tools
            .call_tool("pythia_status", json!({"index_name": "nonexistent"}))
            .await
            .unwrap();
    ...
    async fn test_handle_sync_not_found() {
        ...
        let result = tools
            .call_tool("pythia_sync", json!({"index_name": "nonexistent"}))
            .await;
    ...
    async fn test_handle_add_repo_invalid_url() {
        ...
        let result = tools
            .call_tool("pythia_add_repo", json!({"url": "not-a-valid-url"}))
            .await;
    ...
    async fn test_handle_add_repo_missing_url() {
        ...
        let result = tools.call_tool("pythia_add_repo", json!({})).await;
    ...
    async fn test_handle_sync_missing_index_name() {
        ...
        let result = tools.call_tool("pythia_sync", json!({})).await;
    ...
    async fn test_handle_search_with_top_k() {
        ...
        let result = tools
            .call_tool("pythia_search", json!({"query": "test", "top_k": 5}))
            .await
            .unwrap();
    ...
    fn test_tool_list_schema() {
        ...
        let list_tool = list.iter().find(|t| t.name == "pythia_list").unwrap();
    ...
    fn test_tool_add_repo_schema() {
        ...
        let tool = list.iter().find(|t| t.name == "pythia_add_repo").unwrap();
    ...
    fn test_tool_sync_schema() {
        ...
        let tool = list.iter().find(|t| t.name == "pythia_sync").unwrap();
        let props = &tool.input_schema["properties"];
    ...
    fn test_tool_status_schema() {
        ...
        let tool = list.iter().find(|t| t.name == "pythia_status").unwrap();
        let props = &tool.input_schema["properties"];
        assert!(props["index_name"].is_object());
    ...
    async fn test_handle_status_with_invalid_json() {
        ...
        let result = tools
            .call_tool("pythia_status", json!({"index_name": []}))
            .await;
    ...
    async fn test_all_tools_callable() {
        ...
        let _ = tools.call_tool("pythia_list", json!({})).await;
        let _ = tools.call_tool("pythia_search", json!({"query": "test"})).await;
        let _ = tools.call_tool("pythia_add_repo", json!({"url": "invalid"})).await;
        let _ = tools.call_tool("pythia_sync", json!({"index_name": "test"})).await;
        let _ = tools.call_tool("pythia_status", json!({})).await;
    }
}
