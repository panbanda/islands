...
use pythia_cli::{Config, commands};

#[derive(Parser)]
#[command(name = "pythia")]
#[command(version, about = "Pythia - Codebase Indexing and Inquiry System")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    let filter = if cli.debug {
        EnvFilter::from_default_env().add_directive("pythia=debug".parse()?)
    } else {
        EnvFilter::from_default_env().add_directive("pythia=info".parse()?)
    };
