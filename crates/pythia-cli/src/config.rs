...
impl Config {
    ...
    pub fn from_env() -> Self {
        ...
        if let Ok(val) = std::env::var("PYTHIA_DEBUG") {
            config.debug = val == "1" || val.to_lowercase() == "true";
        }

        if let Ok(val) = std::env::var("PYTHIA_LOG_LEVEL") {
            config.log_level = val;
        }

        if let Ok(val) = std::env::var("PYTHIA_REPOS_PATH") {
            config.indexer.repos_path = PathBuf::from(val);
        }

        if let Ok(val) = std::env::var("PYTHIA_INDEXES_PATH") {
            config.indexer.indexes_path = PathBuf::from(val);
        }

        if let Ok(val) = std::env::var("OPENAI_API_KEY") {
            config.openai_api_key = Some(val);
        } else if let Ok(val) = std::env::var("PYTHIA_OPENAI_API_KEY") {
            config.openai_api_key = Some(val);
        }
...
mod tests {
    ...
    fn test_config_from_file_json() {
        ...
        let file_path = temp_dir.join("pythia_test_config.json");
    ...
    fn test_config_from_file_yaml() {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join("pythia_test_config.yaml");
...
"#;
    ...
    fn test_config_from_file_yml_extension() {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join("pythia_test_config.yml");
...
"#;
    ...
    fn test_config_from_file_invalid_json() {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join("pythia_invalid_config.json");
    ...
    fn test_config_from_file_invalid_yaml() {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join("pythia_invalid_config.yaml");
