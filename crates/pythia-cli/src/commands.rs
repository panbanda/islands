...
use pythia_indexer::service::{IndexerConfig, IndexerService};
use pythia_providers::factory::{create_provider, parse_repo_url};
...
pub async fn list_indexes(config: &Config) -> Result<()> {
    ...
    if indexes.is_empty() {
        output::warning("No indexes found. Use 'pythia add' to add repositories.");
        return Ok(());
    }
...
pub async fn serve_mcp(config: &Config) -> Result<()> {
    use pythia_mcp::server::run_server;

    output::info("Starting Pythia MCP server...");
...
pub async fn interactive_ask(config: &Config) -> Result<()> {
    use pythia_agent::{PythiaAgent, llm::openai::OpenAiProvider};
    ...
    output::info("Pythia Codebase Assistant");
    ...
    loop {
        ...
        println!("\n{}", console::style("Pythia:").magenta().bold());
...
pub async fn show_status(config: &Config) -> Result<()> {
    ...
    println!("\n{}\n", console::style("Pythia Status").bold());
